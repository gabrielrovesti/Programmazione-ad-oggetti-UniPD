...
Esadecagono::Esadecagono(const QVector<Vertice>&V,bool b):PoligonoConvesso(V,16,"Esadecagono",b){
      set_lati(V);
}
...
ostream& operator<< (ostream& os,const Esadecagono& T){
 os<<T.toString().toUtf8().constData();
 return os;
}

void Esadecagono::set_lati(const QVector<Vertice>& v){
  int x=0,y=1;
  while(x<v.size()){
      if(y==v.size())y=0;
...
QString Esadecagono::toString() const{
  QString s ="Esadecagono: ";
  for(auto it=Lista.begin();it!=Lista.end();it++){
    s+= it->toString();
    }
  return s;
}
QString Esadecagono::get_nomeClasse()const{
    return "Esadecagono";
}
double  Esadecagono::get_lati(unsigned int i)const{
  return lati[i];
}
unsigned int Esadecagono::get_nLati()const{
  return nLati;
}
